  ...
  <div>
    <el-card class="box-card" style="width: 400px">
      ...
      <div>
        ...
        <div>用户标签：<el-tag type="success" v-for="(r,index) in admin.roles" :key="index">{{r.nameZh}}</el-tag></div>
        <div style="margin-top: 10px;display: flex;justify-content: space-around">
          <el-button type="danger" @click="showUpdatePasswordView">修改密码</el-button>
          <el-button type="primary" @click="showEditAdminInfoView">修改信息</el-button>
        </div>
      </div>
    </el-card>
    <el-dialog
      ...
      <div>
        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          <el-form-item label="请输入旧密码" prop="oldPass">
            <el-input type="password" v-model="ruleForm.oldPass" aut></el-input>
          </el-form-item>
          <el-form-item label="请输入新密码" prop="pass">
            <el-input type="password" v-model="ruleForm.pass" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item label="请确认新密码" prop="checkNewPass">
...
export default {
  ...
  methods:{
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.postRequest('/system/admin/pass',this.ruleForm).then(resp=>{
            if (resp){
              alert("更新密码成功");
    ...
    updateAdminInfo(){
      this.postRequest('/system/admin/update',this.admin2).then(resp=>{
        if (resp){
