...
<div>
  <div>
    <el-input v-model="jl.name" size="small" placeholder="添加职称等级" prefix-icon="el-icon-plus" style="width: 300px"></el-input>
  ...
  <div style="margin-top: 10px">
    <el-table
        ...
        border
        size="small"
        @selection-change="handlerSelectorChange"
        style="width: 100%">
      <el-table-column
          type="selection"
          width="55">
    ...
    <el-button size="small" style="margin-top: 10px" type="danger" :disabled="this.multipleSelection.length==0" @click="deleteBatch">批量删除</el-button>
  ...
  methods:{
    deleteBatch(){
      this.$confirm('此操作将永久删除['+this.multipleSelection.length+']个职称, 是否继续?', '提示', {
        ...
        this.deleteRequest('/system/basic/joblevel/deleteBatch'+ids).then((resp) => {
          if (resp){
            this.initJls();
          }
        })
    ...
    handlerSelectorChange(val){
      this.multipleSelection = val;
    },
    doUpdate(){
      this.postRequest('/system/basic/joblevel/update',this.updateJl).then(resp=>{
        if (resp){
          this.initJls();
